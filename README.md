...
发件人 → ClawEmail WebSocket / AgentQQ CLI 轮询 → monitor.mjs
                  ...
                  inbox.mjs（助手主动管理：列表/阅读/回复/转发/归档）
                          ↓
            clawemail-backend.mjs（SDK + mail-cli）
...
| `clawemail-backend.mjs` | ClawEmail 封装（SDK 读/写 + mail-cli 列表/移动） |
...
- `xxx@claw.163.com` → `clawemail-backend.mjs`（SDK + mail-cli）
...
|---|---|---|
| list / search | ✅ mail-cli | ✅ agently-cli |
...
```bash
# 列出邮件
node inbox.mjs list kimilophelia@claw.163.com --fid=1 --limit=20
node inbox.mjs list aimilghost@agent.qq.com --limit=20

# 搜索邮件
node inbox.mjs search kimilophelia@claw.163.com "verify" --fid=1
node inbox.mjs search aimilghost@agent.qq.com "Agent"

# 阅读邮件
node inbox.mjs read kimilophelia@claw.163.com <messageId>
node inbox.mjs read aimilghost@agent.qq.com <messageId>
